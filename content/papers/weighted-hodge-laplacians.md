...
Because *d* is untouched, the de Rham complex and its cohomology are exactly what they were. The kernels of the weighted Laplacians are the weighted harmonic spaces, those are isomorphic to the relative and absolute de Rham cohomology, and their dimensions are the ordinary Betti numbers. The weight therefore cannot change the zero-eigenvalue multiplicities; it can change the non-zero spectrum and the associated eigenvectors.
...
State the consequence plainly, because it governs how the experiment should be read: **the harmonic spectrum cannot see the weight at the level of nullity.** Choosing *f* to spotlight one atom changes the non-zero eigenvalues and eigenvectors while leaving the kernel dimension fixed. The paper says as much — the kernels are of the same dimension for every atom, determined by the manifold's topology.

Two smaller notes on the theory. First, these identifications are stated as following by adapting the classical Hodge-theoretic arguments to the weighted setting; the paper contains seven Remarks and no numbered theorem, and the corresponding de Rham–Hodge theory for the drifting Laplacian on non-compact manifolds is Bueler's, already in the literature. This is a formulation paper, not a proof paper, and it does not claim otherwise. Second, the weighted Hodge decomposition it derives is the three-component orthogonal splitting in Eq. 27. It then states that one naturally expects weighted four- and five-component Hodge–Morrey–Friedrichs decompositions, but leaves both unproved; the conclusion names establishing the five-component version as future work.
...
*The numbers are transcribed, and the table does not say what they are.* The baseline figures are copied from three papers rather than recomputed. CAL's cited figures match its own consensus gradient-boosting row exactly, so the CAL column is a consensus model — which makes the honest comparison WHL consensus 0.524 against CAL 0.456, and makes WHL-alone at 0.480 beating a consensus baseline the more interesting result. Nothing in the table says so, and the paper does not state which of its baselines' numbers include their own consensus features; a reader who does not go and check reads five like-for-like columns. The shared split, at least, is real: the mDGL and CAL repositories ship byte-identical 346-protein lists, so "the same dataset" is exact rather than approximate — which cuts against the paper, since with the split released there is nothing standing between it and a like-for-like rerun. Transcription also inherits whatever has happened to the source since: the PSL paper carries an open-access correction published in June 2025, more than a year before this submission, which this paper does not cite. That correction changes the non-blind averages in its Table 1 and explicitly says the blind-prediction results are unchanged, so it does not alter the 0.452 baseline reproduced here; the omission is a citation-provenance problem, not an unresolved numerical one.

*Only one row is blind.* Pooling atoms across proteins before splitting puts a protein's own residues on both sides of the fold boundary. Both rows are described as blind prediction; only the protein-level one is blind to the protein. The gap between roughly 0.86 and 0.48 shows how much easier the random-atom generalization task is and is consistent with substantial optimism from same-protein overlap, but it is not a quantitative decomposition of how much of the atom-level PCC is leakage. The paper's willingness to print both is to its credit — but the atom-level row is not evidence about unseen structures, and the consensus model's margin over mDGL is 0.003.
...
**Not addressed — cost, and reproducibility.** No timings, no complexity discussion, and one Laplacian per atom across 74,000 atoms is not obviously cheap. The eigenvalue computation is MATLAB and no implementation is released; the only link is to the mDGL repository, which carries mDGL's own code, the shared split list, and mDGL's precomputed blind-prediction features, but nothing of this paper's. So the baselines in the table can be rerun and this paper's row cannot, which is the wrong way round for the column making the claim. There is nothing here to package.
